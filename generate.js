...
内容红线（任一触犯即 safe 为 false）：
- 汤底落在自杀方式上，或者需要玩家推出「怎么自杀的」
- 涉及未成年人的性内容或虐待
- 汤底是种族、民族或宗教的刻板印象

只输出 JSON：{"honest":true/false,"solvable":true/false,"safe":true/false,"why":"不通过时说明原因"}`,
    messages: [{ role: "user", content: `汤面：${p.scene}\n汤底：${p.solution}` }]
  });
  const t = extractText(res, "checkHonesty");
