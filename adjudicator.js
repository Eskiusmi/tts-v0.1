...
// 裁判是「查表 + 判断」，不需要重推理。单独一个变量，方便换便宜模型。
// 用 npm run calibrate 对比两个模型的判定质量再决定。
...
// Sonnet 5 起自适应思考默认开启，thinking 块会吃掉 max_tokens 预算；
// 新分词器同样文本还要多约 30% 的 token。预算给小了的话，模型会把额度
// 花在思考上，一个 text 块都吐不出来，拿到的就是空字符串。
// 所以这里必须把「被截断」和「解析失败」分清楚，否则日志会把人带错方向。
export function extractText(res, where) {
  ...
  const kinds = res.content.map((b) => b.type).join(",") || "(空)";
  if (res.stop_reason === "max_tokens") {
    throw new Error(
      `${where}: max_tokens 用尽，没有正文输出（块类型: ${kinds}）。` +
      `调大 max_tokens——思考也算在这个预算里。`
    );
  }
  if (res.stop_reason === "refusal") {
    throw new Error(`${where}: 模型拒绝了这次请求（stop_reason=refusal）。`);
  }
  throw new Error(`${where}: 没有文本输出，stop_reason=${res.stop_reason}，块类型: ${kinds}`);
}
...
预设了、陈述了、或直接问到了该认知本身；只是提到相关的词不算。
玩家一句话里可能同时想到好几个关键点，都要列出。每个 key 全局只报一次，
【已命中】里的 id 不要重复上报。

通关
【核心】是这道题真正要想明白的那一件事。
只要玩家在这一问里用自己的话说出了【核心】的意思，solved 就为 true。
不要求措辞一致，不要求说出细节，不要求提到辅助的关键点
（比如「店员知情」「有人付钱」这类解释性的补充）。
玩家用陈述句还是疑问句都算——「他是不是只有一只手？」和「他只有一只手」
同样说明他想明白了。
不算的情况：只说出【核心】的一部分（比如只说「他不是顾客」但没说他是谁）、
只说出一个辅助关键点、或者复述汤面。
拿不准时问自己：如果玩家现在停下来，他理解这道题了吗？理解了就是 true。
...
【汤面】${puzzle.scene}
【汤底】${puzzle.solution}
【核心】${puzzle.core || puzzle.solution}
【事实】
